# fit a Bayesian GP regression model to critical and known cases data, integrating
# over uncertainty in the true CFR, and return the posterior mean and 95% CI
run_bayesian_model <- function (data, percentageCritical = 3,
                                percentageCriticalRange = c(2, 4),
  ...
  true_cfr_mean <- percentageCritical
  true_cfr_sigma <- mean(abs(percentageCriticalRange - percentageCritical)) / 1.96
  ...
  log_expected_critical <-
    log(baseline_cfr_perc / 100) + log(data$cases_known) - log(reporting_rate) 
  expected_critical <- exp(log_expected_critical)
  
  # define sampling distribution
  greta::distribution(data$critical) <- greta::poisson(expected_critical)
